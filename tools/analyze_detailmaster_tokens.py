...
# --- 設定 ---
output_path = "./data-juicer/outputs/DetailMaster_Dataset_analysis.json" # パスを適宜修正
...
sns.histplot(df['st1_qwen_vl_tokens'], kde=True, color="purple", label="Qwen-VL (Structured)", bins=30, alpha=0.4)
...
plt.title("Detailed Token Count Distribution: Original vs Structured", fontsize=15)
...
# --- 2. 統計サマリーの表示 ---
print("\n--- Statistical Summary ---")
cols = ['clip_tokens', 't5_tokens', 'qwen_vl_tokens', 'st1_qwen_vl_tokens']
...
# --- 4. 構造化による増加量の計算 ---
df['qwen_increase'] = df['st1_qwen_vl_tokens'] - df['qwen_vl_tokens']
print(f"\n--- Structured Prompt Growth ---")
print(f"Average token increase: {df['qwen_increase'].mean():.2f}")
print(f"Max token increase: {df['qwen_increase'].max()}")
